...
(function() {
    ...
    $(".vt_col1").first()
        .append('<tr><th>Total experience&nbsp;(Estimated value)</th><td>' + exp_str + 'ex </td></tr>')
        .append('<tr><th>Player Rating (Estimated)</th><td>' + getTrainingGradeFont(info.trainingGrade) + '&nbsp;&nbsp;(&nbsp;' +
            getScoreGradeFont(info.overallGrade) + 'pts&nbsp;&nbsp;Attribute&nbsp;' +
            getScoreGradeFont(info.propertyGrade) + 'pts&nbsp;&nbsp;Experience3.0&nbsp;' +
            getScoreGradeFont(info.expGrade) + 'pts&nbsp;)</td></tr>');
    // 计算未来经验
    // 可以考虑缓存，当变化时让用户手动更新
    let trainer;
    $.ajax({
    ...
    $(".vt_col1").last()
        ...
        .after('<p>TC Level：' + trainer.level + ' &nbsp;&nbsp;Training effectiveness：' + trainer.base + 'Times &nbsp;&nbsp;Training bonus：+' + trainer.addition + '%xp' + (info.isHardworking ? '&nbsp;&nbsp;Hardworking' : '') + '</p>')
        .after('<h3>Player experience growth estimates</h3>');
    $("#future_exp_table").append("<thead><tr><th>Age</th><th>Main attribute</th><th>Total experience</th><th>3.0Experience Points</th></tr></thead><tbody></tbody>");
    ...
    function getTrainingGradeFont(grade) {
        ...
        const texts = ["Five trainings and below", "Six trainings", "Seven trainings", "Eight trainings", "Nine trainings", "Ten trainings", "Super talent"];
    ...
    function calculateFutureAttr(info, age, future_exp) {
        ...
        if (info.position === "Keeper") attr = (attr + value * 0.05 * 2.7) / 3.7;
